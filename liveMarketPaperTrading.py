# FILE: liveMarketPaperTrading.py (FINAL - Asymmetric Thresholds)
import os
import time
import ccxt
import joblib
...
FEE_PCT = 0.001
SLIPPAGE_PCT = 0.0005
# --- FIX: Asymmetric thresholds for long and short positions ---
LONG_THRESHOLD = 0.57
...
if not API_KEY or not API_SECRET:
    raise SystemExit("Fatal: Set COINEX_API_KEY and COINEX_SECRET env variables.")
    ...
    try:
        ...
        if position > 0:
            ...
            if position_type == "long":
                ...
                if current_price <= sl_price: exit_price = sl_price; reason = 'stop_loss'; closed = True
                elif current_price >= tp_price: exit_price = tp_price; reason = 'take_profit'; closed = True
        ...
        print(f"New 5m candle detected at {new_candles.iloc[-1]['timestamp']}. Analyzing...")
        ...
        if len(prediction_history) > LOOKAHEAD_K:
            ...
            X_old = old_prediction['features']
            price_at_prediction_time = old_prediction['price_at_prediction']
            ground_truth_price = df['close'].iloc[-1]
            y_true_label = 1.0 if ground_truth_price > price_at_prediction_time else 0.0
            ...
            loss.backward()
            optimizer.step()
            print(f"  -> Online learning step complete. Loss: {loss.item():.6f}")

        price = float(new_candles.iloc[-1]['close'])
        print(f"  -> Analysis complete. Model Prob: {prob:.4f} | Balance: ${paper_balance:.2f}")

        if position == 0 and paper_balance > 0:
            usd_to_use = paper_balance * MAX_RISK_PCT
            # --- FIX: Updated entry logic with separate thresholds ---
